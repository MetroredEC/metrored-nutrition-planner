 ...
 * Cloudflare Worker - Microsoft Graph API Proxy
 * 
 * Maneja:
 * - Obtención segura de tokens Azure (client_credentials flow)
 ...
 *   AZURE_TENANT_ID
 *   AZURE_CLIENT_ID
 *   AZURE_CLIENT_SECRET (secret cifrado)
 *   AZURE_SENDER
 */
...
/**
 * Obtiene access token de Azure usando client_credentials flow
 */
...
/**
 * Envía correo vía Microsoft Graph sendMail endpoint
 */
async function sendEmailViaGraph(token, env, { email, subject, html }) {
  try {
    ...
    if (response.ok || response.status === 202) {
      // 202 Accepted es la respuesta estándar de Microsoft
